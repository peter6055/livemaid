...
function GridTextarea({
    ...
    return (
        <textarea
            ...
            className={`block min-w-0 flex-1 resize-none overflow-hidden break-words bg-transparent px-2.5 py-2.5 align-top text-sm leading-snug outline-none placeholder:text-muted-foreground/60 ${mono ? "font-mono" : ""
    ...
    return (
        <div ref={wrapRef} className="h-full">
            <button
                ...
                className={`flex h-full w-full items-center justify-between gap-1 px-2.5 py-2 text-left text-sm outline-none ${value ? "font-mono text-foreground" : "text-muted-foreground/60"
                    }`}
            >
                ...
                >
                    {ER_KEY_OPTIONS.map((opt) => {
                        ...
                        return (
                            ...
                            >
                                <span
                                    className={`flex h-4 w-4 shrink-0 items-center justify-center rounded border ${on ? "border-indigo-500 bg-indigo-500 text-white" : "border-border"
                                        }`}
                                >
                    ...
                    return (
                        <div key={`attr-${i}`} className="border-b border-border last:border-b-0">
                            <div
                                ...
                                className={`group flex items-stretch ${isDropTarget ? "bg-indigo-500/10 ring-1 ring-inset ring-indigo-500/60" : ""
                                    } ${rowError ? "bg-red-500/5 ring-1 ring-inset ring-red-500/50" : "hover:bg-accent/40"} ${dragFrom === i ? "opacity-40" : ""
                                    }`}
                            >
