...
import { Boxes, CircleDot, Diamond, Merge, Split, Square } from "lucide-react";
 ...
 * a single transition. Rendered outside the TransformWrapper at canvasShell level (viewport coords),
 * dark-themed to match the other diagram menus. Closes on outside click or Escape.
 */
export function StateConnectMenu({ state, onPick, onClose }: StateConnectMenuProps) {
  ...
  return (
    <div
      ...
      className="absolute z-50 flex w-56 flex-col overflow-hidden rounded-xl border border-white/10 bg-[#100f1b] text-slate-100 shadow-xl"
      style={{
      ...
      <div className="grid grid-cols-3 gap-2 p-2 pt-0">
        {SHAPES.map((s) => (
          <button
            ...
            className="flex h-14 flex-col items-center justify-center gap-1 rounded-lg border border-white/10 bg-white/[0.03] p-1 text-slate-100 hover:border-indigo-400 hover:bg-white/[0.07]"
          >
            {s.icon}
