...
- **Lifeline Handle Overlay**: In sequence diagrams, hovering near a vertical lifeline surfaces a stack of draggable `+` handles along valid anchor slots. Handles use a high-contrast indigo style with stronger icon weight and are scale-locked so they keep constant physical size while zooming.
